...
// InternalSecretSyncer creates or updates external secrets (e.g. Ramen S3 profiles) from an internal secret.
type InternalSecretSyncer func(ctx context.Context, rc client.Client, scheme *runtime.Scheme, currentNamespace string, secret *corev1.Secret, mirrorPeer *multiclusterv1alpha1.MirrorPeer, logger *slog.Logger) error
...
type MirrorPeerReconciler struct {
	...
	SyncInternalSecret InternalSecretSyncer
	TestEnvFile        string
...
func (r *MirrorPeerReconciler) reconcilePhases(ctx context.Context, logger *slog.Logger, mirrorPeer *multiclusterv1alpha1.MirrorPeer) (ctrl.Result, error) {
	...
	if mirrorPeer.Spec.ManageS3 {
		for _, peerRef := range mirrorPeer.Spec.Items {
			...
			if r.SyncInternalSecret == nil {
				err = fmt.Errorf("SyncInternalSecret callback is not configured")
				logger.Error("Error in updating S3 profile", "Cluster", peerRef.ClusterName, "error", err)
				mirrorPeer.Status.Message = multiclusterv1alpha1.S3ConfigurationFailed
				return ctrl.Result{}, err
			}

			err = r.SyncInternalSecret(ctx, r.Client, r.Scheme, r.CurrentNamespace, &s3Secret, mirrorPeer, logger)
			if err != nil {
				logger.Error("Error in updating S3 profile", "Cluster", peerRef.ClusterName, "error", err)
				mirrorPeer.Status.Message = multiclusterv1alpha1.S3ConfigurationFailed
				return ctrl.Result{}, err
