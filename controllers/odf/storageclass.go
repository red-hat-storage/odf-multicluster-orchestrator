...
func GetStorageIdsForDefaultStorageClasses(ctx context.Context, c client.Client, scNamespacedName types.NamespacedName, spokeClusterName string) (map[CephType]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error occurred while fetching the default storageclasses: %w", err)
	}
	...
	for _, sc := range storageClasses {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to calculate the storageID of storageclass %s: %w", sc.Name, err)
		}
...
func GetStorageClassType(sc storagev1.StorageClass, storageClusterNamespace string) (CephType, error) {
	...
	case fmt.Sprintf(utils.RBDProvisionerTemplate, storageClusterNamespace):
		return RBDType, nil

	case fmt.Sprintf(utils.CephFSProvisionerTemplate, storageClusterNamespace):
...
func CalculateStorageId(ctx context.Context, c client.Client, sc storagev1.StorageClass, storageClusterNamespacedName types.NamespacedName) (string, error) {
	...
	switch sc.Provisioner {
	case fmt.Sprintf(utils.RBDProvisionerTemplate, storageClusterNamespacedName.Namespace):
		...
		storageId = utils.CalculateMD5Hash([2]string{cephClusterFSID, radosNamespaceName})

	case fmt.Sprintf(utils.CephFSProvisionerTemplate, storageClusterNamespacedName.Namespace):
		storageId = utils.CalculateMD5Hash([2]string{cephClusterFSID, DefaultSubvolumeGroup})
