...
func IsStorageClientType(mirrorPeer *multiclusterv1alpha1.MirrorPeer, clientInfoMap map[string]string) (bool, error) {
	for _, v := range mirrorPeer.Spec.Items {
		peerRefType, err := getPeerRefType(v, clientInfoMap)
		if err != nil {
			return false, err
		}
