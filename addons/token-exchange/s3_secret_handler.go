...
const (
	...
	S3DefaultBlueSecretMatchString = "odrbucket"
...
func (S3SecretHandler) getBlueSecretFilter(obj interface{}) bool {
	...
	if blueSecretMatchString == "" {
		blueSecretMatchString = S3DefaultBlueSecretMatchString
	}
...
func (s S3SecretHandler) createBlueSecret(name string, namespace string, c *blueSecretTokenExchangeAgentController) error {
	...
	// fetch s3 endpoint
	route := &routev1.Route{}
	err = s.spokeClient.Get(context.TODO(), types.NamespacedName{Name: S3RouteName, Namespace: c.agentNamespace}, route)
	if err != nil {
...
func (S3SecretHandler) createGreenSecret(name string, namespace string, c *greenSecretTokenExchangeAgentController) error {
	return nil
}
