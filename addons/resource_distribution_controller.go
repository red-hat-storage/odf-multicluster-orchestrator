	...
	for _, template := range templateList.Items {
		...
		for _, foundConsumer := range storageConsumerList.Items {
			...
			if template.GetLabels()[utils.ObjectKindLabelKey] == "VolumeGroupReplicationClass" {
				vgrc := ocsv1alpha1.VolumeReplicationClassSpec{Name: template.Name}
				index := slices.Index(expectedConsumer.Spec.VolumeReplicationClasses, vgrc)
				if template.DeletionTimestamp.IsZero() {
					if slices.Contains(addVRCToConsumers, foundConsumer.GetName()) {
						logger.Info("Adding VGRC to StorageConsumer", "VGRC", vgrc, "StorageConsumer", foundConsumer.GetName())
						if index == -1 {
							expectedConsumer.Spec.VolumeReplicationClasses = append(expectedConsumer.Spec.VolumeReplicationClasses, vgrc)
						}
					} else {
