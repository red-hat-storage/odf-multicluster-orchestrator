...
func (r *StorageClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.HubClient.Get(ctx, types.NamespacedName{Name: req.Name}, mirrorPeer); err != nil {
		return ctrl.Result{}, fmt.Errorf("failed to fetch MirrorPeer %s: %w", req.Name, err)
	}
