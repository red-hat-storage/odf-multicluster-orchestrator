...
func (r *MirrorPeerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !hasStorageClientRef {
		...
		storageIds := make(map[odf.CephType]string)
		for k, v := range clusterStorageIds[r.SpokeClusterName] {
			storageIds[odf.CephType(k)] = v
		}
...
func labelDefaultStorageClasses(ctx context.Context, logger *slog.Logger, client client.Client, storageClusterName string, storageClusterNamespace string, storageIdsMap map[odf.CephType]string) error {
	storageClasses, err := odf.GetDefaultStorageClasses(ctx, client, storageClusterName)
	if err != nil {
		return err
	}

	for _, sc := range storageClasses {
		storageClassType, err := odf.GetStorageClassType(sc, storageClusterNamespace)
		if err != nil {
			return err
		}
		...
		sc.Labels[fmt.Sprintf(utils.RamenLabelTemplate, utils.StorageIDKey)] = storageIdsMap[storageClassType]
...
func labelDefaultVolumeSnapshotClasses(ctx context.Context, logger *slog.Logger, client client.Client, storageClusterName string, storageClusterNamespace string, storageIdsMap map[odf.CephType]string) error {
	volumesnapshotclasses, err := odf.GetDefaultVolumeSnapshotClasses(ctx, client, storageClusterName)
	...
	for _, vsc := range volumesnapshotclasses {
		volumeSnapshotClassType, err := odf.GetVolumeSnapshotClassType(vsc, storageClusterNamespace)
		if err != nil {
			return err
		}
		...
		vsc.Labels[fmt.Sprintf(utils.RamenLabelTemplate, utils.StorageIDKey)] = storageIdsMap[volumeSnapshotClassType]
...
func (r *MirrorPeerReconciler) fetchClusterStorageIds(ctx context.Context, mp *multiclusterv1alpha1.MirrorPeer, storageClusterNamespacedName types.NamespacedName) (map[string]map[string]string, error) {
	...
	for _, pr := range mp.Spec.Items {
		...
		if r.SpokeClusterName == pr.ClusterName {
			...
			storageIds, err := odf.GetStorageIdsForDefaultStorageClasses(ctx,
				r.SpokeClient,
