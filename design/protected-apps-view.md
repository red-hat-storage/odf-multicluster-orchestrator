...
spec:
  # Application type (determined by reverse mapping)
  type: ApplicationSet  # or "Subscription" or "Discovered"
  
  # Reference to the source application
  applicationRef:
    apiVersion: argoproj.io/v1alpha1
    kind: ApplicationSet
    name: payment-service
  ...
  subscriptionInfo:
    subscriptionRefs:
      - name: web-frontend-sub-1
        namespace: web-app-ns

status:
  observedGeneration: 1
  lastSyncTime: "2025-11-12T10:30:00Z"
  conditions:
    - type: Ready
      status: "True"
```
...
ApplicationSet type: Owner = ApplicationSet CR (when ApplicationSet deleted → view deleted)
